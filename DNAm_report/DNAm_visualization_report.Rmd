---
title: "DNAm visualization report"
output: 
  html_document:
    toc: true
    toc_float: true
...
params:
  ...
  cpg_positions: './annot_rds_files/positions-probesEPIC-hg38.rds'
  annot_cgi: './annot_rds_files/annotCGI-CpG-Islands-cpgIslandExt.rds'
  annot_gene: './annot_rds_files/annotGene_NCBI-RefSeq_refGene.rds'
  annot_repeats: './annot_rds_files/annotRepeats-RepeatMasker-rmsk.rds'
---

<!-- HTML tags to define some styles that can be used in this Rmd -->
...
<!-- All these chunks are embedded into a HTML details flag to avoid warnings to be directly visible to the user but still accessible to check that no warning cause the report to be unreliable. -->
<details>
...
```
```{r retrieve_probes_per_region, echo=FALSE}
# ----
...
if (params$export_tables_graphs) {
  ...
  dir.create(dir_name)
  dir.create(paste0(dir_name, "/", "MethPctPlot_wholeGenome"))
  dir.create(paste0(dir_name, "/", "MethPctPlot_perRegion"))
}
```
...
if (params$export_tables_graphs) {
  write.csv(info_dataframe,
    file = paste0(dir_name, "/", "generalInfoTable.csv"),
    quote = TRUE, row.names = FALSE
  )
}

# Print the dataframe with the general information about the analysis. The table is printed via 3 tables to allow multiple columns. 
...
info_dataframe[6:(5 + floor((nrow(info_dataframe) - 5) / 2)), ] %>%
  kable("html") %>%
    ...
    if (params$export_tables_graphs) {
      dev.copy(pdf,
        paste0(
          ...
          "MethPctPlot_wholeGenome_", name_sample, ".pdf"
        ),
        width = 10, height = 4
      )
      tmp_var <- dev.off()
...
boxplot_meth_distr_per_region(meth_pct_distrib_per_region)
...
if (params$export_tables_graphs) {
  dev.copy(pdf,
    paste0(
      dir_name, "/",
      "MethPctPlot_perRegion_perSample.pdf"
    ),
    width = 10, height = 4
  )
  tmp_var <- dev.off()
...
<!-- Generate and print a dataframe with mean data per region. -->
```{r methdifftable_generate_and_print, echo=FALSE, results = 'asis'}
# Get a dataframe with mean methylation % difference per region. ----
regions_with_methpct <- get_region_mean_diff_dataset(
...
# Print the dataframe with mean methylation % difference per region.----
...
```{r methdifftable_export, echo=FALSE}
# export table?----
if (params$export_tables_graphs) {
  write.csv(regions_with_methpct,
    file = paste0(dir_name, "/", "MethPctDifference_perRegion_Table.csv"),
    quote = TRUE, row.names = FALSE
  )
}
```
  ...
  if (params$export_tables_graphs) {
    ...
    dev.copy(pdf,
      paste0(
        ...
        "MethPctPlot_", name_region_corrected, ".pdf"
      ),
      width = 10, height = 6
    )
    tmp_var <- dev.off()
...
```
<!-- Removing the annotation rds files. -->
```{r removing_annotation, echo=FALSE, message=FALSE}
rm(list = c("annot_gene_list", "annot_cgi_list", "annot_repeats_list"))
```
